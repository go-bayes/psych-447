...
-   The following is useful tutorial for explaining what multi-level modelling is doing under the hood (only read the first five pages). The tutorial would be especially useful for experimentalists: [here](https://www.dropbox.com/s/d2kug8oo6xdihvl/SorensenVasishthMay12014.pdf?dl=0): [@sorensen2015bayesian] The published version is less clear, but still useful [@vasishth2018bayesian]
-   Matti Vuorre provides and explanation for how to do meta-analysis using the BRMS package [here](https://mvuorre.github.io/posts/2016-09-29-bayesian-meta-analysis/).
-   In my view, the clearest account of multilevel modelling is Richard McElreath's Statistical Rethinking[@mcelreath2020].
-   Solomon Kurz's BRMS companion to Statistical Rethinking is really great, and has lots excellent code for making illuminating graphs: [@kurz2020]
-   There is no single multi-level model. To understand how model syntax, heat over the GLMM Faq page [here](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html); For model specification see especially [this](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#model-specification)
-   To understand how to write multi-level models in BRMS, in addition to the Kurz book, see Paul Bürkner's tutorial [here](https://arxiv.org/pdf/1705.11123.pdf)
...
There is much debate about terminology in multi-level modelling. In the circles I travel, the term "mixed-effects model" is preferred. There's some discussion about the terminological issues on the GLMMFAQ page [here](<https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html>).  In part, this is because there are many different multi-level models. In part, the confusion owes to the evolution of nomenclature among different groups. And in part, the confusion owes to people being genuinely confused. Our task here is to walk you through the steps of relatively simple multi-level models, which will be a powerful asset to your work. We want to do more than teach you how to write and interpret these models; we want to foster the intuitions that will help you to understand what they are doing under the hood. 

Regarding nomenclature, we will generally prefer the terms "group-varying intercept" and "group-varying slope" to the terms "random effect" and "random slope." The term "random" tense to invite confusion. However, when we use the terms "group-varying" it will be important to remember that a "group" can include an individual who has been repeatedly measured over time. 
...
Here, `yi` is the observed outcome in effect size units, and `sei` is the observed sampling variances ($\sigma_i^2$). (You can read more about the metafor package [here](https://wviechtb.github.io/metafor/)).
...
The code for a simple meta-analytic model in BRMS is:
...
```{r eval = FALSE, code_folding = FALSE}
lme4::lmer(y ~ intercept + beta1 + beta2  + (beta1|group) + (beta2|group))
```
...
Consider the following example from the parameters package [here](<https://easystats.github.io/parameters/articles/demean.html>)
...
Here we see shrinkage at work. The predicted points are closer to the average response because the model is a little skeptical of grouping structure.
...
I'm not sure I'm sold yet on within and between group "demeaning" because it would appear to lose the benefits of pooling and shrinkage that we get from multilevel models (sometimes called "regularisation").  I imagine there will be increasing discussion about this approach during the next few years.  For this reason I thought you should know about it. 

## Comment on centering

It's almost never a good idea to center your indicators within groups. 
