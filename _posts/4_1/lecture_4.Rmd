...
We report sample descriptive statistics for indicators of personal Kessler-6 distress below in [Table](#k6tab)1.



**Table 1**<a id="k6tab"></a>
```{r k6tab}
library(gtsummary)
tb1 <-nz %>%
  dplyr::select(
...
Note that you can use the `gtsummary` package to create in-line referencing. For example: Average Kessler-6 distress in 2018 was `r gtsummary::inline_text(tb1, variable = KESSLER6sum, column ="2018")` and in 2019 was `r gtsummary::inline_text(tb1, variable = KESSLER6sum, column ="2019")`.
