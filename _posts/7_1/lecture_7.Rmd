...
%>%
  ...
  dplyr::rename(income = Household.INC, # shorter names
                not_male = Male) %>%
  dplyr::mutate(edu_s = scale(Edu)) # standard deviation units for education

df['height_cm_c'] = as.data.frame(scale(df$HLTH.Height, center = TRUE, scale = FALSE)*1000) # work around for a bug - center and convert meter units to cm
...
```{r}
plot(ggeffects::ggpredict(model_theory4, terms = c("not_male","height_cm_c")))
```

What we find is a non-linearity.  The model predicts that, on average, short men are expected to make much less than short women. Among women, height differences are not a reliable predictor of household income. By contrast among main, height differences are a reliable predictor. Women tend to be shorter than men by about 13cm
...
```{r eval = FALSE}
model_theory7 <- lm(income ~ GenCohort + height_cm_c + not_male + edu_s, data = df)
head(df)
```

```{r}
performance::compare_performance(model_theory3)
```
