...
Two variables affect an outcome. Both are correlated with each other. One affects the outcome positively and the other affects the outcome negatively

Consider two correlated variables that jointly predict Kessler 6 (K), religion (R) and political conservativism but one positively and one negatively. 

First consider this relationship, where conservativism causes religion
...
d_sim <- data.frame(K=K,R=R,M=C)
```


First we only condition on conservativism 
...
What if religion were to cause conservativism?
...
```{r}
ms21 <- parameters::model_parameters(
  lm(K ~ C, data = d_sim2)
)
plot(ms21)
ms21
...
```{r}
ms22 <- parameters::model_parameters(
  lm(K ~ R, data = d_sim2)
)
plot(ms22)
ms22
...
```{r}
ms23 <- parameters::model_parameters(
  lm(K  ~ C + R, data = d_sim2)
)
plot(ms23)
ms23
```
...
C <- rnorm( n , U )
K <- rnorm( n , R - C )
d_sim3 <- data.frame(K = K, R = R, U = U, M = M )
```
...
ggdag::ggdag_adjustment_set(dag_m1)
```


```{r}
ms31 <- parameters::model_parameters(
  lm(K  ~ C, data = d_sim3)
)
plot(ms31)
ms31
...
```{r}
ms32 <- parameters::model_parameters(
  lm(K  ~ R, data = d_sim3)
)
plot(ms32)
ms32
...
```{r}
ms33 <- parameters::model_parameters(
  lm(K  ~ C + R, data = d_sim3)
)
plot(ms33)
ms33
```
...
The New Zealand Attitudes and Values Study that is relevant to evaluating the theory of whether prototypicality of Māori group membership "moderates" the effect of perceived discrimination on ethnic identity in Māori.^[I place "moderates" in shutter quotations because it is not clear what this means: moderates as in predicts or moderates as in causes.] 
