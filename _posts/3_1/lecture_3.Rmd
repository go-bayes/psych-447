...
In this lecture we'll first introduce you to the ggplot2 package, and vocabulary, for creating graphs in R. We'll be following the approach described in the book "R for data science" which can be found [here](https://r4ds.had.co.nz/data-visualisation.html).

We'll then turn to data-wrangling


## Creating a granph 

Step 1, load the ggplot2 package by loading the tidyverse package
...
Step 3. Inspect the relationship between highway fuel efficiency and a cars engine size (which is given by the variable `displ`).
...
A basic problem with this graph is that we don't know what it is representing. To avoid this problem, it is useful to get into the habit of adding titles to your graphs, and also of using informative axis labels. We do this by adding layers
...
Let's walk through the logic of the ggplot2 "grammar of graphis" 
...
We use facet_grid for graphing the relationship between two variables. 

Note the difference betwen these two graphs:

Here the focus is on the relationship between class and the x variable, displacement

```{r}

ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, color = class)) +
  facet_grid(class ~ .)  + theme(legend.position = "bottom") 
```
...
We can focus on relationship between class and the x and y variables simultaneously. Here we add the 'year` indicator and we do not see much of an improvement in highway milage for the different classes, adjusting for displacement:

```{r fig.height=12}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, color = class)) +
  facet_grid(class ~ year) + theme(legend.position = "bottom") +
  labs(title = "Relationship bewtween engine displacement and fuel efficiency by clas in the mpg automobile dataset") + 
  xlab("Engine displacement in (units)") + 
  ylab("Highway miles per liter") 
```
...
ggplot(data = mpg) +
  geom_smooth(mapping = aes(x = displ, y = hwy)) + theme(legend.position = "bottom") +
  labs(title = "Relationship bewtween engine displacement and fuel efficiency in the mpg automobile dataset") +
  xlab("Engine displacement in (units)") +
  ylab("Highway miles per liter") 
```
