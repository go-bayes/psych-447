...
Missing data poses a problem. Missingness biases inference. However missingness it is not an intractable problem. If we assume that the mechanism giving rise to the missing data is random *conditional on known features of the data,* we can use regression to predict missing values. Statisticians call the assumption of multiple imputation: "MAR: Missing at Random" [\^1]. The language is admittedly confusing -- what statisticians mean is "missing conditional on predictors." Where MAR assumptions are satisfied we may generate multiple datasets with predictions for the missing observations. Because regression is uncertain, we predict multiple responses for missing observations. This practice of generating multiple datasets is called Multiple Imputation. Before proceeding with the mechanics of multiple imputation, we take a moment to return to fundamental principles that we have been discussing throughout this course:

1.  The practice of statistical inference is, at its most general level, inherently an attempt to move from known observations in our sample to inferences about unknown parameters of a population that is not observed. Statistical inference is, in core respects, educated guesswork; the practice of multiple imputation is continuous with a data-scientist's larger mission to guess wisely[\^1]: For a explanation, see: [here](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4121561/#:~:text='Missing%20completely%20at%20random'%20means,missing%20and%20observed%20blood%20pressures.)
