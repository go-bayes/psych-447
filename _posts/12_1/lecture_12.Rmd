...
loop_lmer_model_tab <- function(x) {
  ...
  out <- parameters::pool_parameters(mp, effects = "all")
  return(out)
}
...
```{r huxtable_latextable, include = FALSE}
library(huxtable)
huxtable::as_hux(m_list)%>%
...
Let's look at the results from therow-wise deleted data:
...
Mixinng is a problem, and the coefficient plot reveals a mess:
...
We can model missing-ness in BRMS in one step (for continuous missing variables)
