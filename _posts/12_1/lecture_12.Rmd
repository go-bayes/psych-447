...
Missing data poses a problem. Missingness biases inference. However missingness it is not an intractable problem. If we assume that the mechanism giving rise to the missing data is random *conditional on known features of the data,* we can multiply impute datasets - which involves predicting missing values. Statisticians call the assumption of multiple imputation: "MAR: Missing at Random" [\^1]. The language is admittedly confusing -- what statisticians mean is "missing conditional on predictors" however, practically speaking the key point is that we can use the sample to predict outcomes for the population from which the sample is drawn. Not only do we better integrate over our uncertainty about the population better when we multiply impute outcomes, doing so enables us to address selection bias arise from stratifying on only that portion of the population that returns responses. Before proceeding with the mechanics of mutliple imputation, take a moment to consider some critical general observations that we have been discussing throughout this course:

1.  The practice of statistical inference is, at its most general level, inherently an attempt to move from known observations in our sample to inferences about unknown parameters of a population that is not observed. Statistical inference is guesswork; the practice of multiple imputation is continuous with a data-scientists larger mission to guess wisely[\^1]: For a wonderful explanation, see: [here](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4121561/#:~:text='Missing%20completely%20at%20random'%20means,missing%20and%20observed%20blood%20pressures.)

2.  We have made a distinction between regression for prediction and regression for causal inference. When we regress to predict it is generally useful to include all indicators in our dataset that might be related to the outcome. We have discussed strategies for assessing improvements in model fit using specific fit criteria like the BIC criterion, the WAIC criterion, and LOO.

Gelman and Hill give the following advice about inclusion of variables when we are using regression to predict [@gelman2006 p.69]:
...
Gelman and Hill give this advice for *prediction*. However, for causal inference we must avoid confounding and obtain an unbaised estimate of an exposure's causal effect on the outcome(s) of interest. As we have seen in lecture 11, causal inference is difficult, and often counter-intuitive. For causal inference we absolutely do not want to take the "Causal Salad" approach that we would take were we interested in prediction. As Richard McElreath writes [\@mcelreath2020a, p.46]:
...
To help assist us with the problems of obtaining unbiased causal inference we have discussed the utility of building causal DAGs, noting that inference is always conditional on the DAG that we have drawn. Frequently, the data under-determine the DAG. There are many DAGs consistent with our observations. Regression is a powerful tool for reliable causal inference but it is not sufficient for reliable inference. \
\
Returning to multiple imputation, when we are imputing we are are *predicting* features of a population, and for this purpose, it is generally useful to follow the Gelman and Hill advice about stratifying across all those indicators of the population that might predict what the responses for the population would have been had they been observed.

### Visualising missingness

Let us visualise in a subset of the the longitudinal NZ dataset
...
No, the intuition was wrong. However, it is important to realise that missingness might not be completely at random conditional on variables in the model. In this case, you dataset cannot avoid its biases by multiple imputation.
