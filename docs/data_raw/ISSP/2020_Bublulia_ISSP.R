...
lon1 <- df %>%
  ...
  rename(t0.ismeasured = t0,
         t1.ismeasured = t1)%>%
  arrange(id)



lon1 <- haven::zap_formats(lon1)
lon1 <- haven::zap_label(lon1)
lon1 <- haven::zap_widths(lon1)
lon1 <- haven::zap_labels(lon1)
str(lon1)

dff <-lon1 %>%
  pivot_longer(!id
  )%>%
  separate(.,"name", into=c("wave","variable"))%>%
  pivot_wider(., id_cols = c("id","wave"), names_from = "variable", values_from = "value")
