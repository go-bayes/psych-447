...
### Question  9  Graph the relationship betweeen `religiousity` (x-axis) and `rightwing`` (y-axis) in the ISSP dataset
...
issp <- issp %>%
  mutate(
    id = factor(id),
    thr_ath = factor(thr_ath),
    thr_bd = factor(thr_bd),
    thr_ch = factor(thr_ch),
    thr_hd = factor(thr_hd),
    thr_jw = factor(thr_jw),
    thr_ms = factor(thr_ms),
    neg_ath = factor(neg_ath),
    neg_bd = factor(neg_bd),
    neg_ch = factor(neg_ch),
    neg_hd  = factor(neg_hd),
    neg_jw = factor(neg_jw),
    neg_ms = factor(neg_ms),
    wave  = factor(wave),
    nzeuro = factor(nzeuro),
    eduyears = numeric(years),
    male = factor(male),
    age = numeric(age),
    rightwing = numeric(rightwing),
    religiosity = numeric(religiosity)
  )
```
...
```{r}
library(ggplot2)
ggplot(data = issp, aes(y = rightwing, x = religiosity, colour = wave)) + 
  stat_smooth(method = lm, fullrange = TRUE, alpha = 0.1) +  geom_point(alpha = .01)
```
