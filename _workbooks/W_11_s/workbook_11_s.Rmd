...
The New Zealand Attitudes and Values Study that is relevant to evaluating the theory of whether prototypicality of Māori group membership "moderates" the effect of perceived discrimination on ethnic identity in Māori.[^1]

[^1]: I place "moderates" in shutter quotations because it is not clear what this means: moderates as in predicts or moderates as in causes.
...
```{r cache=TRUE}
# graph
p_m1 <- plot(conditional_effects(m1,
                            "Relid_c",
                            spaghetti = TRUE,
                            nsamples = 100)
             )[[1]]
...
```{r cache=TRUE}
# graph
p_m2a <- plot(conditional_effects(m2,
                            c("Relid_within"),
                            spaghetti = TRUE,
                            re_formula = NULL,
                            nsamples = 100)
...
```
```{r}
p_m2b <- plot(conditional_effects(m2,
                            c("Relid_between_c"),
                            spaghetti = TRUE,
